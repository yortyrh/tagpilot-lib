...
pub struct AudioTags {
    ...
    pub trackTotal: Option<u32>, // Total number of tracks
    pub albumArtist: Option<String>,
    pub comment: Option<String>,
    pub disc: Option<u32>,
    pub discTotal: Option<u32>, // Total number of discs
}
...
pub fn read_tags(file_path: String) -> Result<AudioTags> {
    ...
    let tagged_file = match Probe::open(path) {
        Ok(probe) => match probe.read() {
            ...
            Err(e) => {
                return Err(napi::Error::from_reason(format!("Failed to read file: {}", e)));
            }
        },
        Err(e) => {
            return Err(napi::Error::from_reason(format!("Failed to open file: {}", e)));
        }
    };
    ...
    if !has_tags {
        return Err(napi::Error::from_reason("File format not supported by lofty"));
    }
    ...
    let tags = if let Some(tag) = primary_tag {
        AudioTags {
            ...
            trackTotal: track_total, // Use track total from track_total() function
            albumArtist: tag.artist().map(|s| s.to_string()), // Using artist as album_artist
            comment: tag.comment().map(|s| s.to_string()),
            disc: tag.disk(),
            discTotal: disc_total, // Use disc total from disk_total() function
        }
    } else {
        ...
        AudioTags {
            ...
            track: track_number, // Use track number from track() function
            trackTotal: track_total, // Use track total from track_total() function
            albumArtist: first_tag.artist().map(|s| s.to_string()), // Using artist as album_artist
